...
const statusColors: Record<Invoice['status'], 'default' | 'info' | 'success' | 'error' | 'warning'> = {
  'Brouillon': 'default',
  'Envoyee': 'info',
  'Partielle': 'warning',
  'Payee': 'success',
...
export function InvoiceViewModal({ isOpen, onClose, invoice, items }: InvoiceViewModalProps) {
  ...
  const handleDownload = async () => {
    ...
    try {
      setDownloading(true);
      await generateInvoicePDF(invoice, items, company, invoice.client);
    } catch (error) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Détails de la facture">
      <div className="space-y-6">
        ...
        <div className="flex items-start justify-between">
          <div>
            <h3 className="text-2xl font-extrabold text-builty-gray">{invoice.invoice_number}</h3>
        ...
        <div className="grid grid-cols-2 gap-4 p-4 bg-builty-gray-lighter rounded-lg">
          ...
          <div>
            <p className="text-sm text-gray-600 mb-1">Date échéance</p>
            <p className="font-medium text-gray-900">
              {new Date(invoice.due_date).toLocaleDateString('fr-FR')}
            </p>
          </div>
          <div>
            <p className="text-sm text-gray-600 mb-1">Conditions paiement</p>
            <p className="font-medium text-gray-900">{invoice.payment_terms || '30 jours'}</p>
          </div>
          <div>
        ...
        <div>
          <h4 className="font-semibold text-gray-900 mb-3">Lignes de la facture</h4>
        ...
        <div className="border-t pt-4 space-y-2">
          <div className="flex justify-between text-sm">
            ...
            <span className="font-semibold">{invoice.amount_ht?.toFixed(2)} €</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-gray-600">TVA:</span>
            <span className="font-semibold">{invoice.tva_amount?.toFixed(2)} €</span>
          </div>
          <div className="flex justify-between text-lg font-bold border-t pt-2">
            <span>Total TTC:</span>
            <span style={{ color: primaryColor }}>{invoice.amount_ttc?.toFixed(2)} €</span>
          </div>
          <div className="flex justify-between text-sm">
